...
local function cacheRequirePlay()
...
musicDetail.functions.fetchMusicDetail = function ()
    if not main_state.option(2) or main_state.option(1030) then
        musicDetail.functions.clearViewData()
        return 0
    end
    return musicDetail.functions._fetchMusicDetail(MUSIC_DETAIL.GET_WAIT_TIME, main_state.timer(11))
end

musicDetail.functions.fetchMusicDetailOnPlay = function ()
    return musicDetail.functions._fetchMusicDetail(0, 0)
end

musicDetail.functions._fetchMusicDetail = function (waitTime, time)
    ...
    -- キャッシュがなければ通信して取得する
    local nowTime = (main_state.time() - time) / 1000
    -- 楽曲情報を表示開始までは消す
    if nowTime < waitTime then
        musicDetail.functions.clearViewData()
        return 0
    end
...
musicDetail.functions.loadPlay = function ()
    local wasSuccessLoadCache, _ = pcall(cacheRequirePlay)
...
musicDetail.functions.loadSelect = function ()
